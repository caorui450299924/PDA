package com.android.barcode;

public class BK_info_mh {
    private String wl;
    private String lch;
    private String jjc;
    private String hwh;
    private String pz;
    private String gg;
    private String xs;
    private String cpbm;
    private String kbh;
    private String gdh;
    private String gys;
    private String czfl;
    private String bmcl;

    public BK_info_mh(String wl, String lch, String jjc, String hwh, String pz, String gg, String xs, String cpbm, String kbh, String gdh, String gys, String czfl, String bmcl) {
        this.wl = wl;
        this.lch = lch;
        this.jjc = jjc;
        this.hwh = hwh;
        this.pz = pz;
        this.gg = gg;
        this.xs = xs;
        this.cpbm = cpbm;
        this.kbh = kbh;
        this.gdh = gdh;
        this.gys = gys;
        this.czfl = czfl;
        this.bmcl = bmcl;
    }

    public String getWl() {
        return wl;
    }

    public void setWl(String wl) {
        this.wl = wl;
    }

    public String getLch() {
        return lch;
    }

    public void setLch(String lch) {
        this.lch = lch;
    }

    public String getJjc() {
        return jjc;
    }

    public void setJjc(String jjc) {
        this.jjc = jjc;
    }

    public String getHwh() {
        return hwh;
    }

    public void setHwh(String hwh) {
        this.hwh = hwh;
    }

    public String getPz() {
        return pz;
    }

    public void setPz(String pz) {
        this.pz = pz;
    }

    public String getGg() {
        return gg;
    }

    public void setGg(String gg) {
        this.gg = gg;
    }

    public String getXs() {
        return xs;
    }

    public void setXs(String xs) {
        this.xs = xs;
    }

    public String getCpbm() {
        return cpbm;
    }

    public void setCpbm(String cpbm) {
        this.cpbm = cpbm;
    }

    public String getKbh() {
        return kbh;
    }

    public void setKbh(String kbh) {
        this.kbh = kbh;
    }

    public String getGdh() {
        return gdh;
    }

    public void setGdh(String gdh) {
        this.gdh = gdh;
    }

    public String getGys() {
        return gys;
    }

    public void setGys(String gys) {
        this.gys = gys;
    }

    public String getCzfl() {
        return czfl;
    }

    public void setCzfl(String czfl) {
        this.czfl = czfl;
    }

    public String getBmcl() {
        return bmcl;
    }

    public void setBmcl(String bmcl) {
        this.bmcl = bmcl;
    }
}

